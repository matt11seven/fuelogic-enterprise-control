...
async function getRequestUserIdOrFail(req, res) {
  ...
  if (!userId) {
    res.status(400).json({ message: 'UsuÃ¡rio invÃ¡lido para operar pedidos' });
    return null;
  }
  return userId;
}
...
// GET /api/orders/stats - MÃ©tricas de pedidos
...
// POST /api/orders/bulk - Criar mÃºltiplos pedidos de um batch
router.post('/bulk', async (req, res) => {
  try {
    ...
    if (!orders || !Array.isArray(orders) || orders.length === 0) {
      return res.status(400).json({ message: 'Lista de pedidos invÃ¡lida' });
    }
...
router.patch('/:id/truck', async (req, res) => {
  try {
    ...
    console.error('Erro ao vincular caminhÃ£o:', error);
    res.status(500).json({ message: 'Erro ao vincular caminhÃ£o' });
  }
});

// PATCH /api/orders/:id/delivery-estimate - Definir previsÃ£o de entrega
router.patch('/:id/delivery-estimate', async (req, res) => {
  try {
    ...
    console.error('Erro ao definir previsÃ£o de entrega:', error);
    res.status(500).json({ message: 'Erro ao definir previsÃ£o de entrega' });
  }
});
...
router.post('/:id/notes', async (req, res) => {
  try {
    ...
    if (!note) {
      return res.status(400).json({ message: 'Nota nÃ£o pode ser vazia' });
    }
